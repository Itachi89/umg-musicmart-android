...
public class SalesRecordDetailAsyncTask extends MusicMartFragmentAsyncTask<Map<String, Object>, List<SalesRecordDetail>> {


    public static final String SALES_RECORD_ID = "SALES_RECORD_ID";

    @Override
    protected List<SalesRecordDetail> doInBackground(Map... parameters) {
        Log.d(this.getClass().toString(), ">>>>>>Inside SalesRecordListTask - doInBackground");
        List<SalesRecordDetail> list = null;
        String username = "restclient";
        String password = "D3Rfg$gbmi^1Ydv3f*B";
        String unp = username+":"+password;
        String result = null;
        String category =  (String) parameters[0].get(SALES_RECORD_ID);

        String url= "http://192.168.1.6:8080/umg-musicmart-web-services/rest/salesrecorddetail/"+ category +"?deviceId=5D26E4C6915A4301A08369348698A620FFFFFFFF";
        Log.d("urls", "url: " + url);
        ...
        try{
            list =  (new ObjectMapper()).readValue(result, new TypeReference<List<SalesRecordDetail>>() {});
        }
        catch (Exception e){
